...
def load_system_prompt_from_config(config_dict: dict) -> Result[str, CoreError]:
    ...
    try:
        ...
        if not prompt_path.exists():
            ...
            return Err(
                CoreError(f"System prompt file not found: {prompt_path}", context)
            )
        ...
        if not prompt_content.strip():
            context = ErrorContext.create(
                ...
                input_data={"prompt_file": system_prompt_file},
                suggestions=[
                    "Add content to system prompt file",
                    "File exists but is empty",
                ],
            )
            return Err(
                CoreError(f"System prompt file is empty: {prompt_path}", context)
            )
        ...
        context = ErrorContext.create(
            "load_system_prompt",
            input_data={
                "config_dict_keys": list(config_dict.get("llm_processing", {}).keys())
            },
            suggestions=[
    ...
    try:
        with log_feature("openai_llm_processing") as feature:
            ...
            messages = [
                ...
                (
                    "user",
                    f"Please process the following YouTube transcript:\n\n{transcript_obj.transkript}",
                ),
            ]

            feature.add_metric(
                "total_input_length",
                len(system_prompt) + len(transcript_obj.transkript),
            )
...
def process_with_anthropic(
    ...
    try:
        ...
        context = ErrorContext.create(
            ...
            input_data={"error_type": type(e).__name__},
            suggestions=[
                "Check Anthropic library installation",
                "Verify config_dict structure",
            ],
        )
...
def process_with_google(
    ...
    try:
        ...
        context = ErrorContext.create(
            ...
            input_data={"error_type": type(e).__name__},
            suggestions=[
                "Check Google GenAI library installation",
                "Verify config_dict structure",
            ],
        )
...
@log_function(log_performance=True)
def process_transcript_with_llm_dict(
    transcript_obj: TranskriptObject, config_dict: dict
) -> Result[TranskriptObject, CoreError]:
    """
    ...
    try:
        ...
        context = ErrorContext.create(
            ...
            suggestions=[
                "Check config_dict structure",
                "Ensure llm_processing section exists",
            ],
        )
    ...
    if provider not in provider_functions:
        context = ErrorContext.create(
            ...
            input_data={
                "provider": provider,
                "available_providers": list(provider_functions.keys()),
            },
            suggestions=[
    ...
    if isinstance(result, Ok):
        ...
        logger.info(
            ...
            extra={
                ...
                "response_length": len(processed_obj.bearbeiteter_transkript)
                if processed_obj.bearbeiteter_transkript
                else 0,
            },
        )
    else:
...
if __name__ == "__main__":
    ...
    if not available_providers["openai"]:
        print(
            "❌ OpenAI nicht verfügbar - Install: pip install langchain-openai langchain-community"
        )
    ...
    print(
        f"✅ Available: {[p for p, available in available_providers.items() if available]}"
    )
    print(
        f"❌ Missing: {[p for p, available in available_providers.items() if not available]}"
    )
