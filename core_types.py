...
class ErrorContext:
    ...
    def create(cls, operation: str, **kwargs) -> ErrorContext:
        ...
        return cls(
            ...
            if kwargs.get("include_trace", False)
            else None,
...
def validate_score(score: float) -> Result[float, ValidationError]:
    ...
    context = ErrorContext.create(
        "validate_score", input_data={"score": score, "type": type(score).__name__}
    )
    ...
    if not 0.0 <= score <= 1.0:
        context.suggestions = [
            "Provide value between 0.0 and 1.0",
            "Check if percentage needs conversion",
        ]
        return Err(
            ValidationError(f"Score must be between 0.0 and 1.0, got {score}", context)
        )
...
def validate_path(path: Path) -> Result[Path, FileSystemError]:
    ...
    """
    context = ErrorContext.create(
        "validate_path", input_data={"path": str(path), "type": type(path).__name__}
    )
...
def validate_range(
    ...
    context = ErrorContext.create(
        "validate_range", input_data={"value": value, "min": min_val, "max": max_val}
    )
    ...
    if not min_val <= value <= max_val:
        ...
        return Err(
            ValidationError(
                f"Value {value} not in range [{min_val}, {max_val}]", context
            )
        )
