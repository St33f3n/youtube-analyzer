...
from core_types import Result, Ok, Err, is_ok, is_err, unwrap_ok, unwrap_err, CoreError
...
class FeatureLogger:
    ...
    def checkpoint(self, step_name: str, context: Optional[Dict[str, Any]] = None) -> None:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb) -> None:
        ...
        if self.total_steps is not None:
            completion_context.update({
                ...
                'completion_rate': round(self.steps_completed / self.total_steps * 100, 2)
            })
        ...
        if self.expected_duration:
            ...
            completion_context['performance_category'] = (
                "faster_than_expected" if performance_ratio < 0.8 else
                "as_expected" if performance_ratio < 1.2 else
        ...
        if exc_type:
            ...
            self.log.info(f"✓ Feature '{self.name}' completed", extra=completion_context)
...
def log_result_function(
...
) -> Callable[[Callable[..., Result[T, Exception]]], Callable[..., Result[T, Exception]]]:
    """
    ...
    """
    def decorator(func: Callable[..., Result[T, Exception]]) -> Callable[..., Result[T, Exception]]:
        @wraps(func)
