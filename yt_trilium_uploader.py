...
class TriliumUploader:
    ...
    def __init__(self, config_dict: dict):
        ...
        if not TRILIUM_PY_AVAILABLE:
            self.logger.error(
                "trilium-py not available - install with: pip install trilium-py"
            )
            return
    ...
    def _init_trilium_client(self) -> Result[ETAPI, CoreError]:
        ...
        with self.client_lock:
            ...
            try:
                ...
                self.logger.info(
                    "Trilium client initialized successfully",
                    extra={
                        "base_url": base_url,
                ...
                return Err(
                    CoreError(f"Failed to initialize Trilium client: {e}", context)
                )

    @log_function("trilium_upload")
    def upload_transcript(
        self, transcript_obj: TranskriptObject
        ...
        if not transcript_obj.bearbeiteter_transkript:
            error = CoreError(
                "Cannot upload to Trilium: No processed transcript available"
            )
        ...
        try:
            ...
            self.logger.debug(
                "Trilium note created successfully",
                extra={
                    "note_id": note_id,
            ...
            attributes_result = self._create_note_attributes(
                client, note_id, transcript_obj
            )
    ...
    def _format_note_content(self, transcript_obj: TranskriptObject) -> str:
        """
        Formats clean note content as HTML with properly formatted timestamps
        ...
        # Original transcript (reference) - jetzt mit formatierten Timestamps
        if transcript_obj.transkript:
            content_parts.append("## Original Transcript")
            formatted_transcript = self._format_transcript_with_timestamps(transcript_obj.transkript)
            content_parts.append(formatted_transcript)
    ...
    def _format_transcript_with_timestamps(self, transcript: str) -> str:
        """
        Formatiert Transkript-Text so dass jeder Timestamp-Abschnitt auf einer neuen Zeile steht
    
        Args:
            transcript: Raw transcript text mit [HH:MM:SS] timestamps
        
        Returns:
            Formatierter Transkript mit Zeilenumbrüchen vor jedem Timestamp
        """
        if not transcript:
            return ""
    
        # Regex pattern für Timestamps: [HH:MM:SS] oder [MM:SS]
        timestamp_pattern = r'(\[\d{1,2}:\d{2}:\d{2}\])'
    
        # Split text bei Timestamps, aber behalte die Timestamps
        ...
        for part in parts:
            ...
            # Ist das ein Timestamp?
            if re.match(timestamp_pattern, part):
                # Wenn wir schon eine Zeile haben, füge sie hinzu
                if current_line.strip():
                    formatted_lines.append(current_line.strip())
                # Starte neue Zeile mit Timestamp
                current_line = part + " "
            else:
                # Füge Text zur aktuellen Zeile hinzu
                current_line += part + " "
    ...
    def _create_note_attributes(
        self, client: ETAPI, note_id: str, transcript_obj: TranskriptObject
        ...
        try:
            ...
            metadata_mapping = {
                ...
                "uploadDate": transcript_obj.upload_date.isoformat()
                if transcript_obj.upload_date
                else "",
                "processingDate": transcript_obj.date_created.isoformat()
                if transcript_obj.date_created
                else "",
            }
            ...
            context = ErrorContext.create(
                ...
                input_data={
                    "note_id": note_id,
                    "metadata_count": len(metadata_mapping),
                },
                suggestions=[
                    "Check note exists",
                    "Verify attribute creation permissions",
                ],
            )
            return Err(CoreError(f"Failed to create note attributes: {e}", context))

    @log_function("create_note_tags")
    def _create_note_tags(
        self, client: ETAPI, note_id: str, trilium_config: dict
    ) -> Result[None, CoreError]:
        """
    ...
    def get_upload_statistics(self) -> Dict[str, Any]:
        ...
        return {
            **self.upload_stats,
            ...
            "attributes_per_note": (
                self.upload_stats["total_attributes_created"]
                / self.upload_stats["total_notes_created"]
                if self.upload_stats["total_notes_created"] > 0
                else 0
            ),
        }
...
def upload_to_trilium_dict(
    transcript_obj: TranskriptObject, config_dict: dict
) -> Result[TranskriptObject, CoreError]:
    """
...
if __name__ == "__main__":
    ...
    if isinstance(config_result, Ok):
        ...
        test_obj.bearbeiteter_transkript = (
            "This is a test LLM-processed transcript with important insights."
        )
        ...
        if isinstance(result, Ok):
            ...
            print("✅ Trilium upload successful:")
