...
class NextcloudUploader:
    ...
    def _init_webdav_client(self) -> Result[WebDAVClient, CoreError]:
        ...
        try:
            ...
            if not password:
                context = ErrorContext.create(
                    ...
                    input_data={
                        ...
                        'nextcloud_password_present': 'nextcloud_password' in resolved_secrets
                    },
                    suggestions=[
            ...
            self.logger.info(
                f"✅ WebDAV client initialized with resolved secrets",
                extra={
                    'base_url': base_url,
                    'username': username,
                    'timeout': 300,
            ...
            context = ErrorContext.create(
                "init_webdav_client",
                input_data={'base_url': self.config_dict.get('storage', {}).get('nextcloud_base_url', 'not_set')},
                suggestions=[
    ...
    def upload_video(self, process_obj: ProcessObject) -> Result[ProcessObject, CoreError]:
        ...
        try:
            with log_feature("nextcloud_upload") as feature:
                ...
                self.logger.info(
                    f"☁️ Starting Nextcloud upload with resolved secrets",
                    extra={
                        ...
                        'secrets_resolved': self.config_dict.get('resolved_secrets', {}).get('nextcloud_password') is not None
                    }
                )
                ...
                if isinstance(create_result, Err):
                    self.logger.warning(f"Directory creation failed: {unwrap_err(create_result).message}")
            ...
            for part in path_parts:
                ...
                try:
                    ...
                    if "already exists" in str(e).lower() or "file exists" in str(e).lower():
                        ...
                        return Err(CoreError(f"Failed to create directory {current_path}: {e}", context))
            
            self.logger.debug(f"Successfully ensured directory path exists: {remote_dir}")
            return Ok(None)
    ...
    def generate_share_link(self, remote_path: str) -> Result[str, CoreError]:
        ...
        try:
            ...
            if not password:
                return self._generate_fallback_link(remote_path, "Password not resolved")
            ...
            if "/remote.php/dav/files/" not in base_url:
                return self._generate_fallback_link(remote_path, "Invalid base_url format")
            
            nextcloud_base = base_url.split("/remote.php/dav/files/")[0]  # "https://nextcloud.example.com"
            ...
            share_api_url = f"{nextcloud_base}/ocs/v2.php/apps/files_sharing/api/v1/shares"
            
            self.logger.debug(
                f"Attempting Share API",
                extra={
            ...
            if response.status_code == 200:
                try:
                    ...
                    if 'ocs' in json_data and 'data' in json_data['ocs'] and 'url' in json_data['ocs']['data']:
                        share_url = json_data['ocs']['data']['url']
                        self.logger.info(f"✅ Share link generated via API: {share_url}")
                        return Ok(share_url)
                except:
                    # Try XML fallback
                    try:
                        ...
                        if url_element is not None:
                            ...
                            self.logger.info(f"✅ Share link generated via API (XML): {share_url}")
                            return Ok(share_url)
                    except:
                        pass
            
            # Share API failed - use fallback
            return self._generate_fallback_link(remote_path, f"Share API failed (HTTP {response.status_code})")
            
        except Exception as e:
            # Exception during Share API - use fallback
            return self._generate_fallback_link(remote_path, f"Share API exception: {e}")
    
    def _generate_fallback_link(self, remote_path: str, reason: str) -> Result[str, CoreError]:
        ...
        try:
            ...
            context = ErrorContext.create(
                ...
                suggestions=["Check nextcloud_base_url configuration", "Verify URL format"]
            )
...
def upload_to_nextcloud_for_process_object_dict(process_obj: ProcessObject, config_dict: dict) -> Result[ProcessObject, CoreError]:
    """
...
if __name__ == "__main__":
    ...
    if isinstance(config_result, Ok):
        ...
        config_dict['resolved_secrets'] = {
            ...
            'nextcloud_password': unwrap_ok(nextcloud_password_result) if isinstance(nextcloud_password_result, Ok) else None
        }
