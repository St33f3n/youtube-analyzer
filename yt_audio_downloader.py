...
class YouTubeAudioDownloader:
    ...
    def download_audio(self, process_obj: ProcessObject) -> Result[ProcessObject, CoreError]:
        """
        ...
        try:
            with log_feature("audio_download") as feature:
                ...
                if isinstance(audio_format, dict):
                    self.logger.warning(f"audio_format is dict, using 'mp3': {audio_format}")
                    audio_format = 'mp3'
                ...
                # Debug ydl_opts
                self.logger.info(
                    f"yt-dlp options (ultra-simplified)",
                    extra={
                ...
                download_result = self._download_with_retries(process_obj.original_url, ydl_opts, max_retries=3)
                ...
                if not actual_path.exists():
                    ...
                    return Err(CoreError(f"Downloaded file not found: {actual_path}", context))
                ...
                if file_size < 1000:  # Less than 1KB probably indicates failure
                    context = ErrorContext.create(
                        ...
                        suggestions=["Check video availability", "Verify audio stream exists", "Check yt-dlp version"]
                    )
                    return Err(CoreError(f"Downloaded file too small ({file_size} bytes)", context))
    ...
    def _download_with_retries(self, url: str, ydl_opts: Dict[str, Any], max_retries: int = 3) -> Result[Path, CoreError]:
            ...
            try:
                ...
                for ext in audio_extensions:
                    ...
                    if expected_file.exists():
                        self.logger.info(f"✅ Found downloaded file (exact match): {expected_file}")
                ...
                for file_path in all_files:
                    if (file_path.name.startswith(expected_base_name) and 
                        ...
                        self.logger.info(f"✅ Found downloaded file (prefix match): {file_path}")
                        return Ok(file_path)
                ...
                if newest_file:
                    self.logger.info(f"✅ Found downloaded file (newest): {newest_file}")
                ...
                self.logger.error(
                    ...
                    extra={
                        ...
                        'audio_files': [f.name for f in all_files if f.suffix.lower() in audio_extensions],
                        'recent_files': [f.name for f in all_files if f.stat().st_mtime > current_time - 600]
                    }
                )
                
                raise FileNotFoundError(f"Downloaded audio file not found. Expected: {expected_base_name}.*")
    ...
    def cleanup_temp_files(self, process_obj: ProcessObject) -> Result[None, CoreError]:
        ...
        try:
            ...
            self.logger.warning(f"Failed to cleanup temp file {process_obj.temp_audio_path}: {e}")
...
def download_audio_for_process_object(process_obj: ProcessObject, config: AppConfig) -> Result[ProcessObject, CoreError]:
    """
    ...
    if isinstance(config_result, Ok):
        ...
        if isinstance(result, Ok):
            ...
            if downloaded_obj.temp_audio_path.exists():
                print(f"  File size: {downloaded_obj.temp_audio_path.stat().st_size / (1024*1024):.1f} MB")
        else:
