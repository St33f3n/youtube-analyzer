...
class RuleAnalysis(BaseModel):
    ...
    confidence: float = Field(
        ge=0.0, le=1.0, description="Konfidenz der Bewertung (0.0-1.0)"
    )
    score: float = Field(
        ge=0.0, le=1.0, description="Qualitäts-Score der Erfüllung (0.0-1.0)"
    )
...
class AnalysisResult(BaseModel):
    ...
    overall_confidence: float = Field(
        ge=0.0, le=1.0, description="Durchschnittliche Konfidenz"
    )
...
class OllamaAnalysisEngine:
    ...
    def __init__(self, config: AppConfig):
        ...
        if not LANGCHAIN_AVAILABLE:
            self.logger.error(
                "Langchain not available - install with: pip install langchain-ollama langchain"
            )
            return
    ...
    def _init_ollama(self) -> Result[None, CoreError]:
        ...
        if not LANGCHAIN_AVAILABLE:
            context = ErrorContext.create(
                "init_ollama",
                suggestions=[
                    "Install langchain: pip install langchain-ollama langchain"
                ],
            )
    ...
    def analyze_transcript(
        self, transcript: str, video_title: str
            ...
            with log_feature("transcript_analysis") as feature:
                ...
                if original_length > 30000:
                    truncated_transcript += (
                        "\n\n[TRANSCRIPT TRUNCATED - ORIGINAL LENGTH: {}]".format(
                            original_length
                        )
                    )
...
{{
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    ...
                    self.logger.debug(
                        f"JSON parsing successful on attempt {attempt + 1}"
                    )
                ...
                self.logger.warning(
                    f"Ollama error on attempt {attempt + 1}, retrying: {e}"
                )
    ...
    def _finalize_analysis(self, result: AnalysisResult) -> AnalysisResult:
        ...
        result.recommendation = (
            result.overall_score >= self.config.scoring.threshold
            and result.overall_confidence >= self.config.scoring.min_confidence
        )
...
def analyze_process_object(
    ...
    # Analyze transcript
    analysis_result = engine.analyze_transcript(
        process_obj.transkript, process_obj.titel
    )
...
if __name__ == "__main__":
    ...
    if isinstance(config_result, Ok):
        ...
        if isinstance(result, Ok):
            ...
            print(
                f"  Recommendation: {'DOWNLOAD' if analyzed_obj.passed_analysis else 'SKIP'}"
            )
