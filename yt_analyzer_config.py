...
class SecretManager:
    ...
    def _setup_keepassxc_backend(self) -> None:
        ...
        try:
            ...
            self.logger.warning(
                "SecretService backend not available, using default keyring"
            )
    ...
    def get_secret(self, service_name: str, username: str) -> Result[str, CoreError]:
        ...
        try:
            ...
            if secret is None:
                ...
                return Err(
                    CoreError(f"Secret not found: {service_name}/{username}", context)
                )
    ...
    def test_secret_access(
        ...
        try:
            ...
            if secret is None:
                return Err(
                    CoreError(f"Secret not accessible: {service_name}/{username}")
                )

            self.logger.debug(
                f"Secret access test successful for {service_name}/{username}"
            )
            return Ok(True)
...
class SecretsConfig(BaseModel):
    ...
    nextcloud_service: str = Field(
        description="Keyring Service-Name für Nextcloud Password"
    )
    nextcloud_username: str = Field(
        description="Keyring Username für Nextcloud Password"
    )
    ...
    anthropic_service: str = Field(
        description="Keyring Service-Name für Anthropic API Key"
    )
    anthropic_username: str = Field(
        description="Keyring Username für Anthropic API Key"
    )
...
class WhisperConfig(BaseModel):
    ...
    language: Optional[str] = Field(
        default=None, description="Sprache forcieren (None = auto-detect)"
    )
...
class ScoringConfig(BaseModel):
    ...
    threshold: float = Field(
        ge=0.0, le=1.0, description="Mindest-Score für Video Download"
    )
    min_confidence: float = Field(
        ge=0.0, le=1.0, description="Mindest-Konfidenz der KI-Bewertung"
    )
...
class StorageConfig(BaseModel):
    ...
    trilium_parent_note: str = Field(
        description="Parent Note in Trilium für YouTube-Inhalte"
    )
...
class ProcessingConfig(BaseModel):
    ...
    temp_folder: Path = Field(
        default=Path("/tmp/youtube_analyzer"), description="Temp-Ordner für Downloads"
    )
    max_video_length: int = Field(
        default=7200, gt=0, description="Max Video-Länge in Sekunden"
    )
...
class LLMProcessingConfig(BaseModel):
    ...
    provider: Literal["openai", "anthropic", "google"] = Field(
        default="openai", description="LLM Provider auswählen"
    )
    model: str = Field(
        default="gpt-4",
    ...
    temperature: float = Field(
        default=0.1, ge=0.0, le=2.0, description="Model Temperature für Konsistenz"
    )
    max_tokens: int = Field(default=4000, gt=0, description="Maximum Output Tokens")
    retry_attempts: int = Field(
        default=5, gt=0, description="Anzahl Retry-Versuche bei API-Fehlern"
    )
    retry_delay: int = Field(
        default=5, gt=0, description="Sekunden zwischen Retry-Versuchen"
    )
    timeout: int = Field(default=300, gt=0, description="Request Timeout in Sekunden")
...
class TrilliumUploadConfig(BaseModel):
    ...
    base_url: str = Field(
        default="https://trilium.example.com", description="Trilium Base URL"
    )
    ...
    note_template: str = Field(
        default="transcript", description="Template für Note-Erstellung"
    )
    ...
    include_metadata: bool = Field(
        default=True, description="LLM-Metadaten in Note einbetten"
    )
    timeout: int = Field(default=60, gt=0, description="Upload Timeout in Sekunden")
...
class AppConfig(BaseModel):
    ...
    def validate_total_weights(cls, v):
        ...
        if total_weight > 1.0:
            raise ValueError(
                f"Total weight of enabled rules ({total_weight}) exceeds 1.0"
            )
        return v
    ...
    def validate_llm_config(cls, v):
        ...
        if isinstance(validation_result, Err):
            raise ValueError(
                f"LLM config validation failed: {validation_result.error.message}"
            )
        return v
...
class SecureConfigManager:
    ...
    def load_config(self) -> Result[AppConfig, CoreError]:
        ...
        try:
            ...
            if not self.config_path.exists():
                ...
                return Err(
                    CoreError(
                        f"Config file created at {self.config_path}. Please edit and add secrets to KeePassXC."
                    )
                )
    ...
    def _validate_all_secrets(self) -> Result[None, CoreError]:
        ...
        for service, username, name in core_secrets_to_test:
            ...
            if isinstance(test_result, Err):
                ...
                self.logger.warning(
                    f"{name} secret not accessible: {test_result.error.message}"
                )
        ...
        if active_provider in provider_secret_mapping:
            ...
            if isinstance(test_result, Err):
                failed_secrets.append(f"{name}: {service}/{username}")
                self.logger.error(
                    f"Active LLM provider secret not accessible: {test_result.error.message}"
                )

        # Core secrets failure = hard error
        core_failures = [
            s
            for s in failed_secrets
            if any(core in s for core in ["Trilium", "Nextcloud"])
        ]
        if core_failures:
            context = ErrorContext.create(
            ...
            return Err(
                CoreError(f"Core secrets not accessible: {core_failures}", context)
            )
        ...
        llm_failures = [
            s
            for s in failed_secrets
            if any(provider in s for provider in ["OpenAI", "Anthropic", "Google"])
        ]
        if llm_failures:
            context = ErrorContext.create(
            ...
            return Err(
                CoreError(
                    f"Active LLM provider secret not accessible: {active_provider}",
                    context,
                )
            )
    ...
    def get_trilium_token(self) -> Result[str, CoreError]:
        ...
        return self.secret_manager.get_secret(
            self.config.secrets.trilium_service, self.config.secrets.trilium_username
        )
    ...
    def get_openai_api_key(self) -> Result[str, CoreError]:
        ...
        return self.secret_manager.get_secret(
            self.config.secrets.openai_service, self.config.secrets.openai_username
        )
    ...
    def get_google_api_key(self) -> Result[str, CoreError]:
        ...
        return self.secret_manager.get_secret(
            self.config.secrets.google_service, self.config.secrets.google_username
        )
    ...
    def _generate_example_config(self) -> None:
        ...
        example_config = {
            ...
            "scoring": {"threshold": 0.7, "min_confidence": 0.6},
            "storage": {
    ...
    def get_system_prompt(self) -> Result[str, CoreError]:
        ...
        try:
            ...
            if not prompt_path.exists():
                ...
                return Err(
                    CoreError(
                        f"LLM system prompt file not found: {prompt_path}", context
                    )
                )
            ...
            self.logger.info(
                "LLM system prompt loaded and cached",
                extra={
            ...
            context = ErrorContext.create(
                "load_llm_system_prompt",
                input_data={
                    "prompt_file": self.config.llm_processing.system_prompt_file
                },
                suggestions=["Check file permissions", "Verify file encoding"],
            )
...
if __name__ == "__main__":
    ...
    if isinstance(config_result, Ok):
        ...
        if isinstance(api_key_result, Ok):
            ...
            print(
                f"✅ {config.llm_processing.provider.title()} API key accessible: {len(api_key)} characters"
            )
        else:
            print(
                f"⚠️ {config.llm_processing.provider.title()} API key not accessible: {api_key_result.error.message}"
            )
