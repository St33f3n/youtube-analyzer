...
class YouTubeVideoDownloader:
        ...
        try:
            with log_feature("video_download") as feature:
                ...
                download_result = self._download_with_retries(
                    process_obj.original_url, ydl_opts, max_retries=2
                )
    ...
    def _download_with_retries(
        self, url: str, ydl_opts: Dict[str, Any], max_retries: int = 2
        ...
        for attempt in range(max_retries):
            try:
                ...
                raise FileNotFoundError(
                    f"Downloaded video file not found. Expected: {expected_base_name}.*"
                )
                ...
                return Err(
                    CoreError(
                        f"yt-dlp video download failed after {max_retries} attempts: {e}"
                    )
                )
